...
# from src.modeling.load_uniFormerV2 import get_uniformer_model
from src.modeling.load_dualFormer import get_uniformer_model
...
def train(args, train_dataloader, val_dataloader, model, tokenizer, training_saver, optimizer, scheduler):
    ...
    for iteration, (img_keys, batch, meta_data) in enumerate(train_dataloader):
        ...
        if backward_now:
            ...
            if (
                    args.save_steps > 0 and global_step % args.save_steps == 0) or global_step == max_global_step or global_step == 1:
                ...
                if args.evaluate_during_training:
                    ...
                    if is_main_process():
                        ...
                        eval_log.append(res)
                        with open(op.join(args.output_dir, args.val_yaml.replace('/', '_') + 'eval_logs.json'),
                                  'w') as f:
...
def update_existing_config_for_inference(args):
    ...
    eval_model_dir = "models/table1/msvd/best-checkpoint/"
    ...
    train_args.eval_model_dir = eval_model_dir
    train_args.resume_checkpoint = eval_model_dir + 'model.bin'
    train_args.model_name_or_path = 'models/captioning/bert-base-uncased/'
    train_args.do_train = True
    train_args.do_eval = True
    train_args.do_test = True
    train_args.config = "src/configs/VidSwinBert/msvd_8frm_default.json"
    train_args.train_yaml = "datasets/MSVD/train.yaml"
    train_args.val_yaml = "datasets/MSVD/val.yaml"
    train_args.output_dir = "output"
    train_args.per_gpu_train_batch_size =6
    train_args.per_gpu_eval_batch_size  = 6
    train_args.num_train_epochs = 15
    train_args.mask_prob  = 0.5
    train_args.max_masked_token = 45
    train_args.mixed_precision_method = "deepspeed"
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--loss_sparse_w', type=float, default=0.5)
    ...
    parser.add_argument('--resume_checkpoint', type=str, default='output/msvd_8frm_default/checkpoint-0-1/model.bin')
...
def main(args):
    ...
    swin_model = get_uniformer_model(args)
